...
class MainActivity : AppCompatActivity() {
    ...
    private fun initListeners(){
        ...
        btnBorrar.setOnClickListener { borrarUltimoCaracter() }

    }

    /**
     * Primero hacemos un if para saber si estamos en el primer numero o en el segundo una vez sepamos en cual estamos
     * miramos si el numero temporal esta vacio o no,si esta vacio llamaremos a la funcion mensajeError y le pasaremos por parametro
     * el mensaje de error que queremos que de , de lo contrario si vemos que tiene numero que lo miramos con isNotEmpty
     * le decimos que numTemp1 ahora sera igual al que tenia menos el ultimo y mostraremos en en el caso de solo el primer numero
     * por las dos panralla el numero que esta introducido,en el caso de que estemos en el numero2 sera igual que con el 1
     *
     */
    private fun borrarUltimoCaracter() {
        if (calc.primerNum) {
            if (calc.numTemp1.isNotEmpty()) {
                ...
                // Si no hay números en numTemp1, borra el operador y cambia a primer número
                calc.op = 0
                muestraValor(calc.numTemp1, calc.numTemp1)
            } else {
                mensajeError("No puedes borrar porque no tiene ningún número ni operador")
            }
        } else {
            if (calc.numTemp2.isNotEmpty()) {
                ...
                // Si no hay números en numTemp2, borra el operador y cambia a primer número
                calc.op = 0
                muestraValor(calc.numTemp1, calc.numTemp1)
            } else {
                // Si no hay números en numTemp2 ni en numTemp1, cambia a primer número
                calc.primerNum = true
                muestraValor(calc.numTemp2, calc.numTemp1)
            }
        }
    }
    /**
